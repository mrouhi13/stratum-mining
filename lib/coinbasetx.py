...
class CoinbaseTransaction(halfnode.CTransaction):
    ...
    def __init__(self, timestamper, coinbaser, value, flags, height,
        ...
        if settings.COINDAEMON_REWARD == 'POS' and ntime is not None:
            self.nTime = ntime

        if settings.COINDAEMON_TX:
            self.strTxComment = "http://github.com/mrouhi13/stratum-mining"
