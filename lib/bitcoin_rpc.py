...
class BitcoinRPC(object):
    ...
    def submitblock(self, block_hex, hash_hex, scrypt_hex):
        # try 5 times? 500 Internal Server Error could mean random error
        # or that TX messages setting is wrong
        attempts = 0
        while True:
            attempts += 1
            if self.has_submitblock:
                try:
                    ...
                    if attempts > 4:
                        log.exception(
                            "submitblock failed. Problem Submitting block %s" % str(
                                e))
                ...
                try:
                    log.debug(
                        "Submitting Block with getblocktemplate submit: attempt #" + str(
                            attempts))
                    log.debug([block_hex, ])
                    resp = (yield self._call('getblocktemplate', [
                        {'mode': 'submit', 'data': block_hex}]))
                    break
                except Exception as e:
                    if attempts > 4:
                        log.exception(
                            "getblocktemplate submit failed. Problem Submitting block %s" % str(
                                e))
                        log.exception("Try Enabling TX Messages in config.py!")
                        raise
                    else:
    ...
    def blockexists(self, hash_hex, scrypt_hex):
        ...
        try:
            ...
            try:
                ...
                log.info(
                    "Cannot find block for hash_hex %s or scrypt_hex %s" % hash_hex,
                    scrypt_hex)
                defer.returnValue(False)

        # after we've found the block, check the block with that height
        # in the blockchain to see if hashes match
        try:
            ...
            # cannot get blockhash from height; block was created,
            # so return true
            defer.returnValue(True)
        else:
            log.info("Cannot find block for %s" % hash_hex)
