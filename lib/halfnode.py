...
if settings.COINDAEMON_ALGO == 'scrypt':
    ...
    log.debug(
        "########################################### Loading Quark Support #########################################################")
    import quark_hash
else:
    log.debug(
        "########################################### Loading SHA256 Support ######################################################")
        ...
        def calc_riecoin(self):
            if self.riecoin is None:
                ...
                sha256 = uint256_from_str(
                    SHA256.new(SHA256.new(''.join(r)).digest()).digest())
                self.riecoin = riecoinPoW(sha256,
                                          uint256_from_compact(self.nBits),
                                          self.nNonce)
            return self.riecoin
    else:
        def calc_sha256(self):
...
class msg_version(object):
    ...
    def serialize(self):
        r = [struct.pack("<i", self.nVersion),
             struct.pack("<Q", self.nServices), struct.pack("<q", self.nTime),
             self.addrTo.serialize(), self.addrFrom.serialize(),
             struct.pack("<Q", self.nNonce), ser_string(self.strSubVer),
