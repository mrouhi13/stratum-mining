...
class TemplateRegistry(object):
    ...
    def submit_share(self, job_id, worker_name, session, extranonce1_bin,
        ...
        if settings.COINDAEMON_ALGO == 'riecoin':
            if len(ntime) != 16:
                raise SubmitException(
                    "Incorrect size of ntime. Expected 16 chars")
        else:
            if len(ntime) != 8:
                raise SubmitException(
                    "Incorrect size of ntime. Expected 8 chars")
        ...
        if settings.COINDAEMON_ALGO == 'riecoin':
            if len(nonce) != 64:
                raise SubmitException(
                    "Incorrect size of nonce. Expected 64 chars")
        else:
            if len(nonce) != 8:
                raise SubmitException(
                    "Incorrect size of nonce. Expected 8 chars")
        ...
        if settings.COINDAEMON_ALGO == 'riecoin':
            ntime_bin = (''.join(
                [ntime_bin[(1 - i) * 4:(1 - i) * 4 + 4] for i in range(0, 2)]))
            nonce_bin = (''.join(
                [nonce_bin[(7 - i) * 4:(7 - i) * 4 + 4] for i in range(0, 8)]))
        ...
        if settings.COINDAEMON_ALGO == 'scrypt':
            ...
            hash_bin = yac_scrypt.getPoWHash(''.join(
                [header_bin[i * 4:i * 4 + 4][::-1] for i in range(0, 20)]),
                int(ntime, 16))
        elif settings.COINDAEMON_ALGO == 'quark':
        ...
        if isBlockCandidate:
            ...
            if settings.COINDAEMON_ALGO == 'riecoin':
                block_hash_bin = util.doublesha(''.join(
                    [header_bin[i * 4:i * 4 + 4][::-1] for i in range(0, 28)]))
            else:
                block_hash_bin = util.doublesha(''.join(
                    [header_bin[i * 4:i * 4 + 4][::-1] for i in range(0, 20)]))
