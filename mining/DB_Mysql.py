...
class DB_Mysql():
    def __init__(self):
        ...
        for setting_name in required_settings:
            if not hasattr(settings, setting_name):
                raise ValueError(
                    "%s isn't set, please set in config.py" % setting_name)
    ...
    def execute(self, query, args=None):
        try:
            ...
            log.debug(
                "MySQL connection lost during execute, attempting reconnect")
            self.connect()
            self.dbc = self.dbh.cursor()
    ...
    def executemany(self, query, args=None):
        try:
            ...
            log.debug(
                "MySQL connection lost during executemany, attempting reconnect")
            self.connect()
            self.dbc = self.dbh.cursor()
    ...
    def get_uid(self, id_or_username):
        ...
        self.execute("SELECT `id` FROM `accounts` where username = %s",
                     (uname))
    ...
    def insert_worker(self, account_id, username, password):
        ...
        self.execute(
            query + '(account_id, username, password) VALUES (%s, %s, %s);',
    ...
    def check_tables(self):
        ...
        if data[0] <= 0:
            raise Exception(
                "There is no shares table. Have you imported the schema?")
