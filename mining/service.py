...
class MiningService(GenericService):
    ...
    def add_litecoind(self, *args):
        ...
        Interfaces.template_registry.bitcoin_rpc.add_connection(args[0],
                                                                args[1],
                                                                args[2],
                                                                args[3])
    ...
    def authorize(self, worker_name, worker_password):
        ...
        if Interfaces.worker_manager.authorize(worker_name, worker_password):
            ...
            if settings.ALLOW_EXTERNAL_DIFFICULTY:
                (is_ext_diff, session[
                    'difficulty']) = Interfaces.worker_manager.get_user_difficulty(
                    worker_name)
            ...
            Interfaces.worker_manager.worker_log['authorized'][worker_name] = (
                0, 0, False, session['difficulty'], is_ext_diff,
            ...
            if worker_name in Interfaces.worker_manager.worker_log[
                'authorized']:
                del Interfaces.worker_manager.worker_log['authorized'][
                    worker_name]
            return False
    ...
    def subscribe(self, *args):
        ...
        session[
            'difficulty'] = settings.POOL_TARGET  # Following protocol specs, default diff is 1
    ...
    def submit(self, worker_name, work_id, extranonce2, ntime, nonce):
        ...
        difficulty = session['difficulty']
        if worker_name in Interfaces.worker_manager.job_log and work_id in \
                Interfaces.worker_manager.job_log[worker_name]:
            (job_id, difficulty, job_ts) = \
                Interfaces.worker_manager.job_log[worker_name][work_id]
        else:
            job_ts = Interfaces.timestamper.time()
            Interfaces.worker_manager.job_log.setdefault(worker_name, {})[
                work_id] = (work_id, difficulty, job_ts)
            job_id = work_id
    ...
    authorize.params = [('worker_name', 'string',
                         'Name of the worker, usually in the form of user_login.worker_id.'),
    ...
    submit.params = [('worker_name', 'string',
                      'Name of the worker, usually in the form of user_login.worker_id.'),
                     ('job_id', 'string',
                     ...
                     ('nonce', 'string',
                      '32bit integer, hex-encoded, big-endian'), ]
