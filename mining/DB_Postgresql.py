...
class DB_Postgresql():
    def __init__(self):
        ...
        self.dbh = psycopg2.connect(
            "host='" + settings.DB_PGSQL_HOST + "' dbname='" + settings.DB_PGSQL_DBNAME + "' user='" + settings.DB_PGSQL_USER + \
        ...
        if hasattr(settings, 'PASSWORD_SALT'):
            ...
            raise ValueError(
                "PASSWORD_SALT isn't set, please set in config.py")
    ...
    def updateStats(self, averageOverTime):
        ...
        self.dbc.execute(
            "select username,SUM(difficulty) from shares where time > to_timestamp(%s) group by username",
            [stime])
        total_speed = 0
        for name, shares in self.dbc.fetchall():
            ...
            total_speed += speed
            self.dbc.execute(
                "update pool_worker set speed = %s, alive = 't' where username = %s",
                (speed, name))
        self.dbc.execute(
            "update pool set value = %s where parameter = 'pool_speed'",
            [total_speed])
        self.dbh.commit()
    ...
    def archive_check(self):
        ...
        self.dbc.execute(
            "select time from shares where upstream_result = true order by time limit 1")
    ...
    def archive_found(self, found_time):
        self.dbc.execute(
            "insert into shares_archive_found select * from shares where upstream_result = true and time <= to_timestamp(%s)",
            [found_time])
        self.dbh.commit()

    def archive_to_db(self, found_time):
        self.dbc.execute(
            "insert into shares_archive select * from shares where time <= to_timestamp(%s)",
            [found_time])
        self.dbh.commit()

    def archive_cleanup(self, found_time):
        self.dbc.execute("delete from shares where time <= to_timestamp(%s)",
                         [found_time])
        self.dbh.commit()

    def archive_get_shares(self, found_time):
        self.dbc.execute("select * from shares where time <= to_timestamp(%s)",
                         [found_time])
        return self.dbc
    ...
    def import_shares(self, data):
        ...
        for k, v in enumerate(data):
            if settings.DATABASE_EXTEND:
                ...
                if v[0] in checkin_times:
                    ...
                    checkin_times[v[0]] = {"time": v[4], "shares": 0,
                                           "rejects": 0}
        ...
        if settings.DATABASE_EXTEND:
            self.dbc.execute(
                "select value from pool where parameter = 'round_shares'")
            round_shares = int(self.dbc.fetchone()[0]) + total_shares
            self.dbc.execute(
                "update pool set value = %s where parameter = 'round_shares'",
                [round_shares])
            ...
            if best_diff > round_best_share:
                self.dbc.execute(
                    "update pool set value = %s where parameter = 'round_best_share'",
                    [best_diff])
            ...
            self.dbc.execute(
                "update pool set value = %s where parameter = 'round_progress'",
                [progress])

            for k, v in checkin_times.items():
                self.dbc.execute(
                    "update pool_worker set last_checkin = to_timestamp(%s), total_shares = total_shares + %s, total_rejects = total_rejects + %s where username = %s",
    ...
    def found_block(self, data):
        ...
        self.dbc.execute(
            "update shares set upstream_result = %s, solution = %s where id in (select id from shares where time = to_timestamp(%s) and username = %s limit 1)",
            (bool(data[5]), data[2], data[4], data[0]))
        if settings.DATABASE_EXTEND and data[5] == True:
            self.dbc.execute(
                "update pool_worker set total_found = total_found + 1 where username = %s",
                (data[0],))
            self.dbc.execute(
                "select value from pool where parameter = 'pool_total_found'")
            total_found = int(self.dbc.fetchone()[0]) + 1
            self.dbc.executemany(
                "update pool set value = %s where parameter = %s",
                [(0, 'round_shares'),
                 (0, 'round_progress'),
                 (0, 'round_best_share'),
    ...
    def insert_user(self, username, password):
        ...
        m.update(self.salt)
        self.dbc.execute(
            "insert into pool_worker (username,password) VALUES (%s,%s)",
    ...
    def check_password(self, username, password):
        ...
        m.update(self.salt)
        self.dbc.execute(
            "select COUNT(*) from pool_worker where username = %s and password = %s",
            (username, m.hexdigest()))
        data = self.dbc.fetchone()
        if data[0] > 0:
            return True
    ...
    def update_pool_info(self, pi):
        self.dbc.executemany("update pool set value = %s where parameter = %s",
                             [(pi['blocks'], "bitcoin_blocks"),
    ...
    def check_tables(self):
        ...
        shares_exist = False
        self.dbc.execute(
            "select COUNT(*) from pg_catalog.pg_tables where schemaname = %(schema)s and tablename = 'shares'",
    ...
    def update_tables(self):
        ...
        while version < current_version:
            self.dbc.execute(
                "select value from pool where parameter = 'DB Version'")
            data = self.dbc.fetchone()
            version = int(data[0])
            if version < current_version:
                log.info(
                    "Updating Database from %i to %i" % (version, version + 1))
    ...
    def update_version_1(self):
        if settings.DATABASE_EXTEND == True:
            ...
            self.dbc.execute(
                "create index shares_username ON shares(username)")
            ...
            self.dbc.execute(
                "create index pool_worker_username ON pool_worker(username)")
            self.dbc.execute(
            ...
            self.dbc.execute(
                "insert into pool (parameter,value) VALUES ('DB Version',2)")
        else:
            self.dbc.execute("create table shares" + \
            ...
            self.dbc.execute(
                "create index shares_username ON shares(username)")
            self.dbc.execute(
                "create table pool_worker(id serial,username TEXT, password TEXT)")
            self.dbc.execute(
                "create index pool_worker_username ON pool_worker(username)")
        self.dbh.commit()
    ...
    def update_version_2(self):
        ...
        self.dbc.execute(
            "update pool set value = 3 where parameter = 'DB Version'")
        self.dbh.commit()
    ...
    def update_version_3(self):
        ...
        self.dbc.execute(
            "update pool set value = 4 where parameter = 'DB Version'")
        self.dbh.commit()

    def update_version_4(self):
        log.info("running update 4")
        self.dbc.execute(
            "alter table pool_worker add difficulty INTEGER default 0")
        ...
        self.dbc.execute("create table shares_archive_found" + \
                         ...
                         "block_num INTEGER, prev_block_hash TEXT, useragent TEXT, difficulty INTEGER)")
        self.dbc.execute(
            "update pool set value = 5 where parameter = 'DB Version'")
        self.dbh.commit()
    ...
    def update_version_5(self):
        ...
        self.dbc.execute(
            "CREATE INDEX shares_time_username ON shares(time,username)")
        self.dbc.execute(
            "CREATE INDEX shares_upstreamresult ON shares(upstream_result)")
        self.dbh.commit()

        self.dbc.execute(
            "update pool set value = 6 where parameter = 'DB Version'")
        self.dbh.commit()
    ...
    def update_version_6(self):
        ...
        try:
            ...
            raise Exception(
                "Could not add pgcrypto extension to database. Have you got it installed? Ubuntu is postgresql-contrib")
        self.dbh.commit()
        ...
        self.dbc.execute(
            "UPDATE pool_worker SET password = encode(digest(concat(password, %s), 'sha1'), 'hex') WHERE id > 0",
            [self.salt])
        self.dbh.commit()
        ...
        self.dbc.execute(
            "update pool set value = 7 where parameter = 'DB Version'")
        self.dbh.commit()
