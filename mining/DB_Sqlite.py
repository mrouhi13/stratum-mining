...
class DB_Sqlite():
    ...
    def updateStats(self, averageOverTime):
        ...
        self.dbc.execute(
            "select username,SUM(difficulty) from shares where time > :time group by username",
            {'time': stime})
        total_speed = 0
        sqldata = []
        ...
        self.dbc.executemany(
            "update pool_worker set speed = :speed, alive = 1 where username = :user",
            sqldata)
        self.dbc.execute(
            "update pool set value = :val where parameter = 'pool_speed'",
            {'val': total_speed})
        self.dbh.commit()
    ...
    def archive_check(self):
        ...
        self.dbc.execute(
            "select time from shares where upstream_result = 1 order by time limit 1")
    ...
    def archive_to_db(self, found_time):
        self.dbc.execute(
            "insert into shares_archive select * from shares where time <= :time",
            {'time': found_time})
        self.dbh.commit()

    def archive_cleanup(self, found_time):
        self.dbc.execute("delete from shares where time <= :time",
                         {'time': found_time})
        self.dbc.execute("vacuum")
        self.dbh.commit()

    def archive_get_shares(self, found_time):
        self.dbc.execute("select * from shares where time <= :time",
                         {'time': found_time})
        return self.dbc
    ...
    def import_shares(self, data):
        ...
        for k, v in enumerate(data):
            if settings.DATABASE_EXTEND:
                ...
                sqldata.append(
                    {'time': v[4], 'rem_host': v[6], 'username': v[0],
                     'our_result': v[5], 'upstream_result': 0, 'reason': v[9],
                     'solution': '',
                     'block_num': v[7], 'prev_block_hash': v[8], 'ua': '',
                     'diff': v[3]})
            else:
                sqldata.append(
                    {'time': v[4], 'rem_host': v[6], 'username': v[0],
        ...
        if settings.DATABASE_EXTEND:
            ...
            self.dbc.execute(
                "update pool set value = :val where parameter = 'round_shares'",
                {'val': round_shares})
            ...
            if best_diff > round_best_share:
                self.dbc.execute(
                    "update pool set value = :val where parameter = 'round_best_share'",
                    {'val': best_diff})
            ...
            self.dbc.execute(
                "update pool set value = :val where parameter = 'round_progress'",
                {'val': progress})

            sqldata = []
            for k, v in checkin_times.items():
                sqldata.append(
                    {'last_checkin': v["time"], 'addshares': v["shares"],
                     'addrejects': v["rejects"], 'user': k})

            self.dbc.executemany(
                "update pool_worker set last_checkin = :last_checkin, total_shares = total_shares + :addshares, " + \
                "total_rejects = total_rejects + :addrejects where username = :user",
                sqldata)
        else:
            self.dbc.executemany(
                "insert into shares (time,rem_host,username,our_result,upstream_result,reason,solution) " + \
                "VALUES (:time,:rem_host,:username,:our_result,:upstream_result,:reason,:solution)",
                sqldata)
    ...
    def found_block(self, data):
        ...
        if settings.DATABASE_EXTEND and data[5] == True:
            self.dbc.execute(
                "update pool_worker set total_found = total_found + 1 where username = :user",
                {'user': data[0]})
            self.dbc.execute(
                "select value from pool where parameter = 'pool_total_found'")
            total_found = int(self.dbc.fetchone()[0]) + 1
            self.dbc.executemany(
                "update pool set value = :val where parameter = :parm",
    ...
    def insert_user(self, username, password):
        self.dbc.execute(
            "insert into pool_worker (username,password) VALUES (:user,:pass)",
    ...
    def check_password(self, username, password):
        self.dbc.execute(
            "select COUNT(*) from pool_worker where username = :user and password = :pass",
            {'user': username, 'pass': password})
        data = self.dbc.fetchone()
        if data[0] > 0:
            return True
    ...
    def update_worker_diff(self, username, diff):
        self.dbc.execute(
            "update pool_worker set difficulty = :diff where username = :user",
    ...
    def check_tables(self):
        ...
        if settings.DATABASE_EXTEND == True:
            ...
            self.dbc.execute(
                "create table if not exists pool(parameter TEXT, value TEXT)")

            self.dbc.execute(
                "select COUNT(*) from pool where parameter = 'DB Version'")
            data = self.dbc.fetchone()
            if data[0] <= 0:
                self.dbc.execute(
            ...
            self.dbc.execute("create table if not exists shares" + \
                             "(time DATETIME,rem_host TEXT, username TEXT, our_result INTEGER, upstream_result INTEGER, reason TEXT, solution TEXT)")
            self.dbc.execute(
                "create table if not exists pool_worker(username TEXT, password TEXT)")
            self.dbc.execute(
                "create index if not exists pool_worker_username ON pool_worker(username)")
    ...
    def update_version_2(self):
        ...
        self.dbc.execute(
            "create index if not exists shares_username ON shares(username)")
        self.dbc.execute(
            "create index if not exists pool_worker_username ON pool_worker(username)")
        self.dbc.execute(
    ...
    def update_version_5(self):
        ...
        self.dbc.execute(
            "create table if not exists pool(parameter TEXT, value TEXT, primary key(parameter))")
        ...
        self.dbc.execute(
            "insert into pool_worker select * from pool_worker_old")
